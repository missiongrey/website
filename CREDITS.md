...
published under a licence that permits reuse with credit.

This file lives at the REPO ROOT, beside `README.md`, and not beside the
images in `assets/texture/`: `deploy/build.sh` copies `assets/` whole, so
a register kept there would ship to the public site. It is a repo
document.

Each plate prints two lines under it (stylesheet 33.0): a subject line in
the figcaption voice and the credit under it in the metadata voice. Both
are recorded per entry below, with the full credit, the licence, the
source URL and the acquisition date. Dates live here, not on the pages.
...
- **Subject line printed on the page**, in the figcaption voice under the plate: Ships at anchor in the Gulf of Suez during the Suez Canal blockage
- **Credit line printed on the page**, in the metadata voice under the subject line: NASA Earth Observatory, Landsat data from the U.S. Geological Survey
...
- **On-page subject line (mono figcaption)**: `Night lights over southern
  Spain on the night of the Iberian power blackout`
...
- **What was done to the file**: grayscale conversion (the source is already
  ...
  subsection 33.2 the way every other crop on this site is stated, and the
  declarations there compute to exactly those rows.
...
- **Full credit line**. Astronaut photograph ISS070-E-80842 was acquired
  ...
  70 crew.
- **Printed on the page**. The subject line `Singapore and Johor at night,
  from the International Space Station` in the figcaption register, and
  under it `ISS Crew Earth Observations Facility, NASA Johnson Space
  Center` in the metadata register. The acquisition date is recorded here
  and nowhere on the site.
